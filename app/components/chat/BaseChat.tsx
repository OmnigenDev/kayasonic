...
export const BaseChat = React.forwardRef<HTMLDivElement, BaseChatProps>(
  (
    ...
    // --- START: Default Settings Modifications ---
    // Find Google provider info (assuming structure based on PROVIDER_LIST in constants.ts)
    const googleProvider = (providerList || PROVIDER_LIST).find(p => p.name.toLowerCase() === 'google');
    // Define the desired default model ID
    const defaultGoogleModelId = 'gemini-2.5-flash-preview-04-17'; // Educated guess for the ID

    // Set initial API key state, adding the default Google key if not present
    const initialApiKeys = getApiKeysFromCookies();
    if (googleProvider && !initialApiKeys[googleProvider.name]) {
      initialApiKeys[googleProvider.name] = 'AIzaSyAPf_xoB4dr2qtAaAt_ETGctQgOMpKjCH0'; // Default Google API Key
    }
    // --- END: Default Settings Modifications ---

    const TEXTAREA_MAX_HEIGHT = chatStarted ? 400 : 200;
    const [apiKeys, setApiKeys] = useState<Record<string, string>>(initialApiKeys); // Use modified initial keys
    const [modelList, setModelList] = useState<ModelInfo[]>([]);
    const [isModelSettingsCollapsed, setIsModelSettingsCollapsed] = useState(true); // Default to collapsed
                      ...
                      {() => (
                        <div className={isModelSettingsCollapsed ? 'hidden' : ''}>
                          <ModelSelector
                            ...
                            // Use default Google model/provider if available and no model/provider is passed in props
                            model={model ?? (googleProvider ? defaultGoogleModelId : undefined)}
                            setModel={setModel}
                            modelList={modelList}
                            // Ensure the provider passed is explicitly ProviderInfo | undefined
                            provider={provider ?? (googleProvider as ProviderInfo | undefined)}
